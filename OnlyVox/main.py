...
OnlyVox - Озвучка текста (Telegram Bot)
...
def inline (call) -> None:
    ...
    if (call.data == "mainmenu"):
        markup = InlineKeyboardMarkup();
        for i in range(9):
            btn = InlineKeyboardButton(text=languages_buttons[language][i][0],
                                       callback_data=languages_buttons[language][i][1]);
            markup.add(btn);
        nxt = InlineKeyboardButton(text=nextbtn[language], callback_data="next$9");
        markup.add(nxt)
        ...
        if (index + 9 < len(languages_buttons[language])):
            for i in range(index, index + 8):
                btn = InlineKeyboardButton(text=languages_buttons[language][i][0], callback_data=languages_buttons[language][i][1]);
                markup.add(btn);
            ...
            for i in range(index, len(languages_buttons[language])):
                btn = InlineKeyboardButton(text=languages_buttons[language][i][0], callback_data=languages_buttons[language][i][1]);
                markup.add(btn);
            back = InlineKeyboardButton(text=backbtn[language], callback_data=f"back${index-8}");
            markup.add(back);
    ...
    ### GET TEXT ###
    elif (call.data in callback_data_btns):
        message = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text=send_text_to_voice[language],
                                        parse_mode="html")  # send message
        bot.register_next_step_handler(message, get_speech, call.data)  # register next func
...
def unknown(message):
    ...
    bot.send_message(message.chat.id, text=Unknown[language],
                     parse_mode="html", reply_markup=markup)  # send message to user



""" TEXT CONTENT HANDLER (GET SPEECH) """
@bot.message_handler(content_types=["text"])
def get_speech(message, speech_language: str) -> None:
    # search user in database
    chat_id: int = message.chat.id;
    database = Database(chat_id);
    database.add_user();
    ...
    bot.send_message(chat_id=chat_id, text=start_voice[language], parse_mode="html");

    speech = Speech(speech_language);
    speech.get(message.text);
    filename: str = speech.mp3();
    del speech;

    full_language: str = "";
    for btn in languages_buttons[language]:
        if (btn[1] == speech_language):
            full_language: str = btn[0];
            break;
        else:
            continue;

    bot.send_message(chat_id=chat_id, text=success_voice[language].format(full_language),
                     parse_mode="html");

    bot.send_voice(chat_id=chat_id, voice=open(filename, "rb"))
    bot.send_audio(chat_id=chat_id, audio=open(filename, "rb"))
    os.remove(filename);

    markup = InlineKeyboardMarkup()
    back = InlineKeyboardButton(text=to_the_start_menu[language], callback_data="mainmenu")
    markup.add(back)
    bot.send_message(message.chat.id, text=To_The_Start_Menu[language], parse_mode="html", reply_markup=markup)
...
if (__name__ == "__main__"):
    ...
    print("Bot OnlyVox is running and ready to work...");
